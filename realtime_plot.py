...
class RealTimePlot(QMainWindow):
    def __init__(self):
        ...
        # Set up Selenium Chrome options for linux ! if there is an error, remove the options
        options = Options()
        options.add_argument("--headless")  # Run in headless mode
        options.add_argument("--no-sandbox")  # Needed to  run chrome on wsl
        ...
        # Open the target URL to start fetching data
    ...
    def update_data(self):
        try:
            ...
            self.axis_y.setRange(longitude - 1, longitude + 1) 
